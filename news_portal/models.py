...
class Mail(models.Model): # модель для работы с почтой
    date=models.DateTimeField(null=True) # дата отправления электронной почты
    subscriber=models.ForeignKey(User, on_delete=models.DO_NOTHING)
    message=models.TextField()

    def __str__(self):
        return f'{self.subscriber} : {self.message}'
